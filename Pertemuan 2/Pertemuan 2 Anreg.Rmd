...
data <- read_excel("D:/Kuliah/Semester 6/Asprak Anreg/Minggu 2/NSC-2025.xlsx")
data #menampilkan sebagian data awal
```
...
$\hat{y_i} = \hat{\beta_0} + \hat{\beta_1} x_{i}, i = 1,2, ..., n$ $\hat{\beta_1} = \frac{\sum (x_iy)-\frac{\sum(x_i)\sum(y)}{n}}{\sum(x_i^2)-\frac{\sum(x_i^2)}{n}}$ $\hat{\beta_0} = \bar{y} - \hat{\beta_1} \bar{x}$
...
Tolak H0, b0 signifikan pada taraf nyata 5%
...
$$\hat{Y} = 50.35333 + 2.87498 X_1$$

dengan Y adalah persentase penduduk miskin dan X1 adalah rata-rata lama sekolah.

Didapatkan nilai $\hat\beta_0 = 50.35333$ artinya nilai dugaan rataan indeks pembangunan manusia ketika rata-rata lama sekolah bernilai 0 adalah 50.353, akan tetapi nilai 0 tidak ada dalam selang pengamatan rata-rata lama sekolah sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut.

Nilai $\hat\beta_1 = 2.87498$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika Rata-rata lama sekolah berubah satu tahun adalah sebesar 2.875.

Koefisien determinasi yang dihasilkan adalah 0.8884 atau 88.84% artinya keragaman IPM yang dapat dijelaskan oleh RLS adalah sebesar 88.84%, sisanya yaitu 11.16% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.
